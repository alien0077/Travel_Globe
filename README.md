...
- Flight numbers: the preload panel accepts a local aviationstack API key in Web or the iOS WebView. Successful aviationstack lookups are saved to localStorage as flight-history cache and reused when later API calls fail; manual origin/destination IATA and the offline seed (`CI100`, `BR190`, `FD234`, `FD235`) remain fallbacks.
- Day/night rendering: the globe now uses bundled Earth lights, cloud, and ocean/specular textures, with replay-date seasonal sun declination and UTC-based sun direction. Route-nearby city light points remain as an additional local emphasis layer.
