...
JOB="/private/tmp/travel-globe-route-gap-bridge-reanalysis-7d-v16"
STATUS="$JOB/status.json"
DONE="$JOB/done.json"
FAILED="$JOB/failed.json"
LOG="$JOB/pipeline.log"
STRICT="$ROOT/AviationDB/data/releases/private/observed-routes/adsblol/global-network-7d-025-repaired-2026-08-01-v14"
OUT="$ROOT/AviationDB/data/releases/private/observed-routes/adsblol/global-network-7d-025-repaired-2026-08-01-v16"
PYTHON="/usr/local/bin/python3"

if [[ "${1:-}" == "status" ]]; then cat "$STATUS" 2>/dev/null || printf '{"state":"not_started"}\n'; exit 0; fi
if [[ "${1:-}" == "__worker" ]]; then
  mkdir -p "$JOB" "$OUT"
  [[ ! -s "$DONE" ]] || exit 0
  [[ ! -s "$FAILED" ]] || exit 2
  ...
  fail() { rc=$?; if [[ "$rc" -ne 0 && ! -s "$DONE" ]]; then "$PYTHON" -c 'import json,sys; from datetime import datetime,timezone; from pathlib import Path; f,s,j,l,r=sys.argv[1:]; p={"state":"failed","exitCode":int(r),"updatedAt":datetime.now(timezone.utc).isoformat(),"jobDir":j,"log":l}; [Path(x).with_suffix(Path(x).suffix+".part").write_text(json.dumps(p,ensure_ascii=False,indent=2)+"\n",encoding="utf-8") for x in (f,s)]; [Path(x).with_suffix(Path(x).suffix+".part").replace(x) for x in (f,s)]' "$FAILED" "$STATUS" "$JOB" "$LOG" "$rc"; fi; cleanup; return "$rc"; }
  trap fail EXIT
  exec >> "$LOG" 2>&1
  cd "$ROOT"
  phase() { "$PYTHON" -c 'import json,sys; from datetime import datetime,timezone; from pathlib import Path; p=Path(sys.argv[1]); x={"state":"running","phase":sys.argv[2],"updatedAt":datetime.now(timezone.utc).isoformat(),"jobDir":sys.argv[3],"outputRoot":sys.argv[4],"rawRescan":False}; t=p.with_suffix(p.suffix+".part"); t.write_text(json.dumps(x,ensure_ascii=False,indent=2)+"\n",encoding="utf-8"); t.replace(p)' "$STATUS" "$1" "$JOB" "$OUT"; }
  phase endpoint_bridges
  "$PYTHON" AviationDB/scripts/add_reference_route_gap_bridges_7d.py --network "$STRICT/global-corridor-network.json.gz" --audit "$STRICT/connectivity/global-connectivity-audit.json.gz" --output "$OUT/global-corridor-network.json.gz" --review-output "$OUT/endpoint-bridge-review.json"
  phase khh_validation
...
printf '{"state":"ready","jobDir":"%s","status":"%s"}\n' "$JOB" "$STATUS"
