#!/usr/bin/env python3
"""
越南 eAIP Parser - 將越南 CAAV eAIP HTML 解析為 SQLite

解析 Vietnam ENR 文件（含 template marker 清理）：
  - ENR 4.4 → NavPoints (waypoints)
  - ENR 3.1 → Conventional Navigation Routes + segments
  - ENR 3.2 → Area Navigation (RNAV) Routes + segments

用法：
  python scripts/parse_vietnam.py --dir data/raw/vietnam/2026-06-30
  python scripts/parse_vietnam.py --dir data/raw/vietnam/2026-06-30 --db data/processed/aviation-vn.sqlite
"""
...
def clean_template_markers(text: str) -> str:
    """清除越南 eAIP HTML 中的 template marker。"""
    # 移除 template markers
...
def _airway_uid(designator: str, source_id: str) -> str:
    return _stable_uid("awy", normalize_ident(designator), source_id, _FIR)
    ...
    class _Parser(HTMLParser):
        ...
        def handle_endtag(self, tag: str) -> None:
            if tag in {"td", "th"} and self._row is not None and self._cell is not None:
                raw = "".join(self._cell)
                cleaned = clean_template_markers(raw)
...
def _parse_enr44_points(html: str, source_id: str, issues: list[dict[str, str]]) -> list[NavPoint]:
    """Parse ENR 4.4 significant points from Vietnam eAIP (with template markers)."""
    ...
    for table in _extract_tables(html):
        ...
        for row in table[1:]:
            ...
            try:
                ...
                issues.append({"severity": "warning", "code": "vn-enr44-coord", "message": f"{ident}: {e}"})
                continue
...
def _nav_point_from_row(row: list[str], source_id: str, issues: list[dict[str, str]]) -> NavPoint | None:
    """從 route table 的一列中解析 waypoint（已 clean template markers）。"""
    row_text = " ".join(row)
    ...
    try:
        ...
        issues.append({"severity": "warning", "code": "vn-route-coord", "message": f"{raw_ident}: {e}"})
        return None
...
def _distance_from_row(row_text: str) -> float | None:
    """從已清理的 row text 提取距離 (NM 或 KM)。"""
    # 找 KM 值（越南用 KM）
    km_match = re.search(r"(\d+(?:\.\d+)?)\s*KM", row_text.upper())
    if km_match:
        val = float(km_match.group(1))
        if 0.1 <= val <= 9999:
            return round(val * 0.539957, 2)  # KM → NM
...
def _route_designator(table: list[list[str]]) -> tuple[str | None, str | None]:
    ...
    for row in table[:10]:
        ...
        if cell.upper().startswith(("ROUTE DESIGNATOR", "1")):
            continue
        # Vietnam format: "A1 Route availability (1) H24"
        m = re.match(r"^([A-Z]\d{1,4}[A-Z]?)", cell.upper())
        if m:
            designator = normalize_ident(m.group(1))
            route_type = "ATS"
            if "RNAV" in cell.upper() or "RNP" in cell.upper():
                route_type_m = re.search(r"\(([^)]+)\)", cell)
                if route_type_m:
                    route_type = normalize_ident(route_type_m.group(1))
...
CREATE TABLE IF NOT EXISTS nav_point (
    ...
    region_code TEXT, source_id TEXT NOT NULL, airac_cycle TEXT,
    effective_date TEXT, is_active INTEGER NOT NULL DEFAULT 1
);
CREATE TABLE IF NOT EXISTS airway (
    uid TEXT PRIMARY KEY, designator TEXT NOT NULL, route_type TEXT,
    direction TEXT, country TEXT, fir TEXT, source_id TEXT NOT NULL,
    airac_cycle TEXT, is_active INTEGER NOT NULL DEFAULT 1
);
CREATE TABLE IF NOT EXISTS airway_segment (
    ...
    direction TEXT, source_id TEXT NOT NULL, airac_cycle TEXT
);
...
def _insert_dataset(conn: sqlite3.Connection, dataset: ParsedDataset, source_id: str, cycle: str) -> None:
    conn.execute(
        ...
        (source_id, "Civil Aviation Authority of Viet Nam (CAAV) / VNAIC", "VN",
         "https://aim.vatm.vn/", "eaip_xhtml", cycle, cycle,
    ...
    for awy in dataset.airways:
        conn.execute(
            """INSERT OR REPLACE INTO airway
               (uid, designator, route_type, direction, country, fir, source_id, is_active)
               VALUES (?,?,?,?,?,?,?,1)""",
            (awy.uid, awy.designator, awy.route_type, None,
             awy.country, awy.fir, awy.source_id),
        )
...
def parse_documents(documents: dict[str, str], source_id: str) -> ParsedDataset:
    ...
    for doc_id, html in documents.items():
        if doc_id == "enr_4_4":
            ...
            print(f"  ENR 4.4: {sum(1 for p in dataset.points if p.source_id == source_id)} points")
            continue

        if doc_id in ("enr_3_1", "enr_3_2"):
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Parse Vietnam eAIP HTML into SQLite")
    parser.add_argument("--dir", required=True, help="Raw data directory")
    parser.add_argument("--db", default=None, help="Output SQLite path")
    parser.add_argument("--source-id", default="vietnam", help="Source identifier")
    ...
    db_path = Path(args.db) if args.db else project_root / "data" / "processed" / "aviation-vn.sqlite"
    ...
    if not documents:
        file_map = {
            "enr_4_4": "VV-ENR-4.4-en-GB.html",
            "enr_3_1": "VV-ENR-3.1-en-GB.html",
            "enr_3_2": "VV-ENR-3.2-en-GB.html",
        }
    ...
    if not documents:
        ...
        print(f"  Run: python scripts/download_vietnam.py")
        sys.exit(1)

    cycle = raw_dir.name
    source_id = args.source_id

    print(f"Parsing Vietnam eAIP...")
    ...
    # 列出前 10 條航線
    cursor = conn.execute("SELECT designator FROM airway ORDER BY designator LIMIT 20")
    routes = [r[0] for r in cursor.fetchall()]
    print(f"\n  Sample routes: {', '.join(routes)}")

    conn.close()

    if dataset.issues:
        print(f"\nWarnings ({len(dataset.issues)}):")
