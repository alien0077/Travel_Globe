#!/usr/bin/env python3
"""Portugal eAIP Parser - standalone script."""
...
FIR = "LISBOA"; COUNTRY = "PT"; REGION = "europe"
...
def _xt(html):
    class P(HTMLParser):
        def __init__(s):
            super().__init__(); s.tbl=[]; s._t=None; s._r=None; s._c=None
        def handle_starttag(s,t,a):
            if t=="table": s._t=[]
            elif t=="tr" and s._t is not None: s._r=[]
            elif t in {"td","th"} and s._r is not None: s._c=[]
        def handle_data(s,d):
            if s._c is not None: s._c.append(d)
        def handle_endtag(s,t):
            if t in {"td","th"} and s._r is not None and s._c is not None:
                s._r.append(" ".join("".join(s._c).split())); s._c=None
            elif t=="tr" and s._t is not None and s._r is not None:
                if any(c.strip() for c in s._r): s._t.append(s._r)
                s._r=None
            elif t=="table" and s._t is not None: s.tbl.append(s._t); s._t=None
...
def parse_enr44(html, sid, issues):
    ...
    for tbl in _xt(html):
        ...
        for row in tbl[1:]:
            ...
            if not ident: 
        print(f"DEBUG: no ident for coord {cm.group(0) if cm else '?'} before='{before[:50]}'")
        continue
            try:
                lat_s,lon_s = cm.group(0).split()
                lat=parse_coord(lat_s); lon=parse_coord(lon_s)
            except: continue
            print(f"DEBUG: adding pt {ident}")
...
def main():
    ...
    ap.add_argument("--source-id", default="portugal")
    args = ap.parse_args()
