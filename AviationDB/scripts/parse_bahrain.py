#!/usr/bin/env python3
"""Bahrain eAIP Parser - standalone script."""
...
FIR = "BAHRAIN"; COUNTRY = "BH"; REGION = "middle-east"
...
def _clean_tm(text):
    """Clean template markers from Vietnam/Bahrain format eAIP."""
    import re
    text = re.sub(r'T[A-Z_]+;[A-Z_]+;\d+', '', text)
    text = re.sub(r'<[^>]+>', '', text)
...
def _xt(html):
    ...
    class P(HTMLParser):
        ...
        def handle_data(s,d):
            if s._c is not None: 
                # Clean template markers like Vietnam format
                d = __import__('re').sub(r'T[A-Z_]+;[A-Z_]+;\d+', '', d)
                s._c.append(d)
...
def main():
    ...
    ap.add_argument("--source-id", default="bahrain")
    args = ap.parse_args()
