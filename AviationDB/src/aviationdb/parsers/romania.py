...
ROUTE_HEADER_RE = re.compile(r"(?m)^\s*(?P<designator>[A-Z]{1,2}\d{1,4}[A-Z]?)\s*$")
MAGNETIC_BEARING_RE = re.compile(r"^\s*\d{3}\s*°\s*$")
HEADER_WORDS = {
    "AIP",
    "AIRAC",
    "AMDT",
...
NAVAID_WORDS = {"DME", "DVOR", "DVOR/DME", "NDB", "VOR", "VOR/DME"}


def parse_romania_pdf_documents(documents: dict[str, bytes], source_id: str) -> ParsedDataset:
    ...
    return parse_romania_text_documents(text_documents, source_id)


def parse_romania_text_documents(documents: dict[str, str], source_id: str) -> ParsedDataset:
...
def _extract_pdf_text(content: bytes, document_id: str) -> str:
    try:
        ...
        raise RuntimeError("Romania PDF parsing requires the optional pypdf dependency") from error
...
def _parse_enr44_points(text: str, source_id: str, issues: list[Issue]) -> list[NavPoint]:
    ...
    for line in text.splitlines():
        ...
        try:
            ...
            issues.append(Issue("warning", "romania-enr44-coordinate", str(error), source_id, ident))
            continue
...
def _parse_enr41_navaids(text: str, source_id: str, issues: list[Issue]) -> list[NavPoint]:
    ...
    for index, line in enumerate(lines):
        ...
        try:
            ...
            issues.append(Issue("warning", "romania-enr41-coordinate", str(error), source_id, ident))
            continue
...
def _parse_route_text(
    ...
    headers = [
        ...
        if _is_route_header(match.group("designator"), text[match.end() : match.end() + 80])
    ]
...
def _parse_route_section(
    ...
    if len(route_points) < 2:
        if route_points:
            issues.append(Issue("warning", "romania-route-too-short", designator, source_id))
        return None
    ...
    distances = _segment_distances(section)
    ...
    for sequence, (from_point, to_point) in enumerate(zip(route_points, route_points[1:], strict=False), start=1):
        distance = (
            distances[sequence - 1]
            if sequence - 1 < len(distances)
...
def _route_point_from_coordinate(
    ...
    try:
        ...
        issues.append(Issue("warning", "romania-route-coordinate", str(error), source_id))
        return None
...
def _route_ident_before(lines: list[str], index: int) -> tuple[str | None, str]:
    for offset in range(index - 1, max(-1, index - 6), -1):
        line = lines[offset].strip().lstrip("%+").strip()
        ...
        for candidate in re.findall(r"\b[A-Z][A-Z0-9]{2,6}\b", line.upper()):
            if candidate not in HEADER_WORDS and candidate not in NAVAID_WORDS and not candidate.startswith("FL"):
...
def _navaid_ident_before(lines: list[str], index: int) -> str | None:
    for offset in range(index - 1, max(-1, index - 10), -1):
        ...
        for candidate in reversed(re.findall(r"\b[A-Z][A-Z0-9]{1,4}\b", line.upper())):
...
def _segment_distances(section: str) -> list[float]:
    ...
    for index, line in enumerate(lines):
        if MAGNETIC_BEARING_RE.fullmatch(line):
            for candidate_line in lines[index + 1 : index + 5]:
                match = re.search(r"(?<!FL)(?<!CH)(?<!/)(?<!\d)(\d{1,3}\.\d)\b", candidate_line)
...
def _is_route_header(designator: str, suffix: str) -> bool:
    if designator.startswith("FL"):
        return False
...
def _is_noise_line(line: str) -> bool:
    upper = line.upper()
    return any(word in upper for word in ("CLASS ", "CONTROLLING", "FREQUENCY", "FOR CONTINUATION", "SEGMENT "))
