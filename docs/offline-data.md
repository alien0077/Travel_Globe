...
The download script writes source archives and CSVs to `shared/source-data/`. The geo prepare script writes `shared/offline-packs/core-global/manifest.json`. The airport prepare script transforms OurAirports CSVs into `shared/offline-packs/core-global/airports-index.json` for offline flight preload and airport lookup.
